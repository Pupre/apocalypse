...
- 변경한 파일
  ...
  - `game/tests/unit/test_indoor_director.gd`
  - 무엇을 바꿨는지
    - 아파트에 `계단실 -> 2층 복도 -> 세탁실 -> 201호` 흐름을 추가하고, `101호 열쇠 -> 101호 수색 -> 201호 열쇠 -> 201호` 식의 2단계 루프를 만들었다.
    - 의원에 `처치 준비실`과 `휴게실`을 추가해 접수창구/진료실/약품 보관실 사이에 보조 수색 동선을 만들었다.
    - 사무실에 `회의실`과 `서버실`을 추가해 탕비실/업무 공간/기록보관실 이후 한 단계 더 깊은 보상 구역이 생기도록 했다.
    - `201호 열쇠` 아이템을 추가하고, 세 건물 확장을 테스트로 고정했다.
  - 왜 바꿨는지
    - 외부 다음 단계로 넘어가기 전에, 마트 외 건물도 최소한 `입구 -> 중간 탐색 -> 잠긴 핵심 구역` 문법이 성립해야 실내 시스템이 재사용 가능한지 확인할 수 있기 때문이다.
  - 영향
    - 이제 마트만 깊고 나머지는 얕은 데모였던 상태에서 벗어나, 다른 건물도 각자 한 단계 더 깊은 파밍 판단을 제공하게 됐다.
...
- 마트 외 건물도 각자 고유한 잠금 해제와 사이드 루트를 갖게 되어, 외부에서 어느 건물을 먼저 공략할지 판단할 이유가 생긴다.
