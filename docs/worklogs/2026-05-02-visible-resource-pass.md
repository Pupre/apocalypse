...
## 추가 패스: 실내 행동 결과 스토리 일러스트

사용자가 말한 “중간중간 퀄리티 높은 일러스트가 나올 때 몰입이 확 올라간다”는 방향을 작게 실제 구조로 연결했다. 이번 패스의 목표는 컷신 시스템을 크게 새로 만드는 것이 아니라, 기존 실내 읽기 카드가 중요한 행동 결과를 받았을 때 전용 장면 일러스트로 바뀔 수 있는 재사용 통로를 만드는 것이다.

### 구현 내용

- `resources/ui/master/indoor/indoor_story_mart_shelf_choice.png`를 추가했다.
  - `imagegen`으로 생성한 마트 식품 진열대 선택 장면을 프로젝트 안에서 쓰는 `960x480` 배너로 크롭/리사이즈했다.
  - 장면은 생수, 통조림, 초코바, 부탄가스, 작은 가방 앞에서 손이 멈춘 구도라서, “지금 당장 무엇부터 챙길까”라는 첫날 생존 판단을 강화한다.
- 실내 이벤트 액션 결과에 `result_illustration_asset`을 지정할 수 있게 했다.
  - 현재는 마트 `search_food_aisle` 결과에 연결했다.
  - 이후 병원 약품 보관실, 주거지 문 앞, 위험 수색 성공/실패 같은 순간에도 같은 필드로 확장할 수 있다.
- `IndoorDirector.get_event_illustration_asset()`은 최근 행동 결과 일러스트를 우선 반환하고, 방 이동 시에는 즉시 결과 이미지를 지워 다시 건물/분류 기본 일러스트로 돌아간다.
- `UiKitResolver`와 실내 리소스 매니페스트에 새 스토리 배너를 등록했다.
...
- 이번 구조는 “모든 탐색에 그림을 붙이는 기능”이 아니라, 선택의 의미가 강한 지점에만 장면을 붙이기 위한 출발점이다.
- 다음 후보는 냉장 보관실 한기, 직원 통로 문 강행, 아파트 잠긴 세대 앞, 병원 약품 보관실처럼 위험과 보상이 동시에 읽히는 행동이 적합하다.
