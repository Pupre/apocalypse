...
## 후속 패스: 실내 행동 카드의 선택 전 비교

실내 행동 카드가 위험 행동에서는 이미 `소란 +N`, `체력 -N`, `피로 +N`, `체온 손실 N`을 보여주고 있었지만, 안전하거나 기본적인 탐색은 여전히 “결과에 따라 다음 선택지가 열린다”처럼 추상적으로 읽혔다. 이번 패스에서는 기본 보조 문구도 실제 플레이 판단에 가깝게 바꾸었다.

- 물건과 단서가 함께 걸린 수색은 `물건/단서 확인 · 새 길이 열릴 수 있음`으로 보여준다.
- 단서나 길만 여는 행동은 `단서 확인 · 새 길이 열릴 수 있음`으로 보여준다.
- 일반 수색은 `물건 확인 · 직접 피해 없음`, 단순 확인은 `조용한 확인 · 직접 피해 없음`으로 보여준다.
- 휴식/수면 계열은 회복과 시간 흐름의 대가가 같이 읽히도록 분리했다.

또한 같은 `여기서 할 일` 섹션 안에서는 안전하고 바로 가능한 선택을 먼저, 위험한 지름길을 그다음, 잠긴 대안을 마지막에 배치하도록 정렬했다. 목표는 버튼을 하나씩 눌러 확인하는 UI가 아니라, 유저가 현재 장소에서 무엇이 안전하고 무엇이 대가를 요구하는지 한눈에 비교하는 UI로 가는 것이다.
...
실내 화면은 행동 카드의 분류와 정보량이 좋아졌지만, 선택지가 많아질수록 플레이어가 “지금 먼저 무엇을 눌러야 하지?”를 다시 읽어야 하는 문제가 남아 있었다. 이번 패스에서는 행동 목록 위에 추천 행동 스트립을 추가해, 현재 방에서 가장 부담 없이 흐름을 열 수 있는 행동을 먼저 제안한다.

### 구현 내용

- `IndoorMode`에 `DecisionStrip`을 추가했다.
  - 현재 방의 행동을 `interaction`, `loot`, `move` 순서로 검토한다.
  - 잠긴 행동과 나가기 행동은 추천 대상에서 제외한다.
  - 우선은 위험 없는 행동을 추천하고, 위험 없는 선택지가 없을 때만 위험 행동을 추천한다.
- 추천 문구는 행동 성격에 따라 다르게 나온다.
  - 물건과 단서를 함께 열 수 있는 탐색.
  - 이미 발견된 물건 획득.
  - 더 볼 일이 적을 때의 다음 구역 이동.
  - 위험 행동일 때의 비용 확인 안내.
- 추천 버튼은 같은 `_on_action_pressed()` 경로를 사용한다.
  - 수량 선택이 필요한 보급품이면 기존 수량 선택 오버레이로 이어진다.
  - 일반 행동이면 기존 실내 행동 처리와 같은 방식으로 처리된다.
...
- 이 변경은 “정답을 강요하는 자동 플레이”가 아니라, 긴 선택지 목록을 읽기 전에 첫 판단을 돕는 UX 장치다. 유저는 여전히 아래의 전체 행동 목록에서 다른 선택을 고를 수 있다.
