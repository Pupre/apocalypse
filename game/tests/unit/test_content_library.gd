...
func _run_test() -> void:
	...
	assert_true(items.has("plastic_bag"), "Prototype items should index lightweight hand-carry containers.")
	...
	_assert_item_contract("plastic_bag")
	...
	assert_eq(String(plastic_bag.get("equip_slot", "")), "hand_carry", "Plastic bags should be equippable in the hand-carry slot.")
	assert_true(float(plastic_bag.get("carry_capacity_bonus", 0.0)) > 0.0, "Plastic bags should increase carrying capacity when equipped.")
